...
int main () {
    ...
    for(mpit it = mp.begin(); it != mp.end(); it++) mnr = min(mnr, it->first.first), mxr = max(mxr, it->first.first), mnc = min(mnc, it->first.first), mxc = max(mxc, it->first.first);
    cout << (mxr - mnr + 1) * (mxc - mnc + 1) - m << endl;
    return 0;
}
